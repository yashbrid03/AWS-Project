...
export default function MyBlogs({ signout, user }) {
  ...
  function get() {
    API.get("blogapi", `/blogs`)
      .then((blogRes) => {
        console.log(blogRes);
        console.log(user);
        const items = blogRes.filter((item) => item.author === user);
        console.log(items);
        setBlogs([items]);
      })
      .catch((err) => {
  ...
  function deleteblog(id) {
    API.del("blogapi", `/blogs/object/${id}`)
      .then((res) => {
        console.log(res);
        get();
        NotificationManager.error("Post deleted Successfully", "");
      })
      .catch((err) => {
        console.log(err);
      });
  ...
  return (
    <>
      <Nav signout={signout} />
      <NotificationContainer />
      <div className="text-5xl mt-8 font-semibold text-center ">My Blogs</div>
              ...
              innerList.map((blogObject) => (
                <div class="p-4 lg:w-1/3" key={blogObject.id}>
                  <div class="h-full bg-gray-100 bg-opacity-75 px-8 pt-16 pb-24 rounded-lg overflow-hidden text-center relative">
                    ...
                    <div className="mt-5">
                      <button
                        onClick={() => deleteblog(blogObject.id)}
